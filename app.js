...
這份紀錄必須用研究問題「${t.q}」來解釋。目前可先確認研究方向，但仍需要補齊資料，才能判斷結果是否可靠。

| 項目 | 判定 | 重點 |
|---|---|---|
| 研究問題 | 需對照 | 結果要能直接回答「${t.q}」 |
| 操作變因 | 需確認 | 每次只改變「${t.iv}」 |
| 控制變因 | 需保持一致 | ${t.cv} |
| 量測紀錄 | 需檢查 | 要有數字、單位與每次原始紀錄 |
| 重複試驗 | 需說明 | 寫清楚各組實際重複幾次 |
...
先整理一張原始紀錄表，逐組填入每一次量測值、單位與重複次數，再比較各組平均值。`;
}
render();
