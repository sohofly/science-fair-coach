...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(action==='get'){const {data:projects}=await db.from('research_projects').select('*').eq('student_id',student.id).order('created_at',{ascending:false});const researchId=String(body.researchId||projects?.[0]?.id||'');const [{data:events},{data:experimentRecords},{data:researchPlan},{data:planSuggestions}]=await Promise.all([db.from('thought_events').select('*').eq('student_id',student.id).eq('research_id',researchId).order('created_at'),db.from('experiment_records').select('id,research_id,subtopic_id,record_kind,method,result,file_name,mime_type,ai_review,created_at').eq('student_id',student.id).eq('research_id',researchId).order('created_at'),db.from('research_plans').select('*').eq('student_id',student.id).eq('research_id',researchId).maybeSingle(),db.from('research_plan_suggestions').select('id,research_id,comment,proposed_plan,status,created_at,decided_at').eq('student_id',student.id).eq('research_id',researchId).order('created_at',{ascending:false})]);delete student.pin_hash;return json({student,projects,currentProject:projects?.find((p:any)=>p.id===researchId)||null,events,experimentRecords,researchPlan,planSuggestions,status:new Date(student.active_until)<=new Date()?'read_only':'active'});}
