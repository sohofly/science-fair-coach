...
assert.match(document.querySelector("#app").textContent, /這個主題的子題/);
document.querySelector('#subtopic-form [name="title"]').value = "光源角度分組";
document.querySelector('#subtopic-form [name="question"]').value = "不同光源角度會讓豆苗彎曲角度產生什麼差異？";
document.querySelector("#subtopic-form").requestSubmit();
await new Promise((resolve) => setTimeout(resolve, 0));
assert.match(document.querySelector("#app").textContent, /光源角度分組/);
assert.match(document.querySelector("#app").textContent, /不同光源角度會讓豆苗彎曲角度產生什麼差異/);
assert.match(document.querySelector("#app").textContent, /繼續討論或上傳實驗紀錄/);
assert.match(document.querySelector("#app").textContent, /進入子題/);
click("[data-subtopic-open]");
assert.match(document.querySelector("#app").textContent, /子題研究工作區/);
assert.match(document.querySelector("#app").textContent, /光源角度分組/);
document.querySelector("#discussion-question").value = "這個子題的角度要怎麼分組？";
click("[data-discussion-submit]");
await new Promise((resolve) => setTimeout(resolve, 0));
assert.match(document.querySelector("#app").textContent, /這個子題的角度/);
assert.ok(JSON.parse(dom.window.localStorage.getItem("scienceFairCoachV1")).experimentRecords[0].subtopicId);
click('[data-go="plan"]');
assert.match(document.querySelector("#app").textContent, /查看備援題目／更換題目/);
click('[data-go="experiment"]');
assert.doesNotMatch(document.querySelector("#app").textContent, /這個子題的角度/);
assert.match(document.querySelector("#app").textContent, /繼續和 AI 教練討論/);
assert.match(document.querySelector("#app").textContent, /支援 JPG、PNG、WEBP、GIF、PDF、CSV、TXT、XLSX；單檔上限 8 MB/);
...
click("[data-record-submit]");
await new Promise((resolve) => setTimeout(resolve, 0));
assert.match(document.querySelector("#app").textContent, /AI 教練回覆/);
click('[data-go="plan"]');
...
document.querySelector("#reflection-form").requestSubmit();
await new Promise((resolve) => setTimeout(resolve, 0));
...
const portalDom = new JSDOM(portalHtml, {
  url: "https://example.test/portal.html",
  runScripts: "dangerously",
});
portalDom.window.eval(fs.readFileSync("config.js", "utf8"));
portalDom.window.fetch = async () => ({
  ok: true,
...
assert.match(studentApi, /PBKDF2/);
assert.match(studentApi, /嘗試次數過多/);
assert.match(studentApi, /action==='list_students'/);
assert.match(studentApi, /select\('student_code'\)/);
assert.match(studentApi, /reflection_added/);
const portalJs = fs.readFileSync("portal.js", "utf8");
assert.match(portalJs, /使用 Email 與密碼登入/);
assert.doesNotMatch(portalJs, /舊學生代號/);
assert.match(portalJs, /發現問題/);
assert.match(portalJs, /拆解問題/);
assert.match(portalJs, /用證據判斷/);
assert.match(portalJs, /反思與負責/);
assert.match(portalJs, /學生的原始想法/);
assert.match(portalJs, /教師或系統提問/);
assert.match(portalJs, /學生提供的證據/);
assert.match(portalJs, /想法如何改變/);
assert.match(portalJs, /目前仍需追問之處/);
assert.match(portalJs, /證據充足/);
...
assert.match(portalJs, /AI 修改建議/);
assert.match(portalJs, /尚未寫入資料庫/);
assert.match(fs.readFileSync("portal.css", "utf8"), /teacher-plan \.portal-form textarea/);
const teacherAccountFunction = fs.readFileSync("supabase/functions/teacher-student-account/index.ts", "utf8");
assert.match(teacherAccountFunction, /ai_plan_suggestion/);
assert.match(teacherAccountFunction, /OPENAI_API_KEY/);
...
assert.match(studentApi, /\(count\|\|0\)>=5/);
assert.match(studentApi, /\(count\|\|0\)<5/);
assert.match(studentApi, /action==='delete_project'/);
...
assert.match(discussionMigration, /record_kind/);
assert.match(discussionMigration, /discussion/);
...
assert.match(fs.readFileSync("admin.html", "utf8"), /總管理者/);
assert.match(fs.readFileSync("admin.js", "utf8"), /所有教師、班級與學生/);
...
assert.match(fs.readFileSync("admin.js", "utf8"), /trim\(\)\.toLowerCase\(\)/, "管理者帳號應忽略大小寫與前後空白");
const cloudflareWorker = fs.readFileSync("cloudflare/src/index.js", "utf8");
assert.match(cloudflareWorker, /lower\(username\)=\?/, "Worker 管理者登入應忽略帳號大小寫");
